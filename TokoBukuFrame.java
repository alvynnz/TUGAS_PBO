...
public class TokoBukuFrame extends JFrame {
    ...
    private JPanel createTransaksiPanel() {
        ...
        DefaultTableModel modelInventori = new DefaultTableModel(
                new String[]{"ID", "Judul", "Harga", "Stok", "Kategori"}, 0
        ) {
            @Override public boolean isCellEditable(int r, int c) { return false; }
        };

        JTable tblInventori = new JTable(modelInventori);
        styleTable(tblInventori);

        JScrollPane spInventori = new JScrollPane(tblInventori);
        spInventori.getViewport().setBackground(TABLE_ROW_BG);

        isiInventoriKeModel(modelInventori);
        ...
        btnTambah.addActionListener(e -> {
            int row = tblInventori.getSelectedRow();
            if (row == -1) {
                JOptionPane.showMessageDialog(this, "Pilih buku dulu.", "Info", JOptionPane.WARNING_MESSAGE);
                return;
            }

            String id = modelInventori.getValueAt(row, 0).toString();
            String judul = modelInventori.getValueAt(row, 1).toString();
            double harga = Double.parseDouble(modelInventori.getValueAt(row, 2).toString());
            int stok = Integer.parseInt(modelInventori.getValueAt(row, 3).toString());
        ...
        btnBayar.addActionListener(e -> {
            ...
            Transaksi_penjualan trx = new Transaksi_penjualan(generateKodeTransaksi(), getNamaKasir());
            ...
            // simpan transaksi -> laporan/omset
            ...
            muatDataKeTabel(); // refresh buku utama
            modelInventori.setRowCount(0);
            isiInventoriKeModel(modelInventori); // refresh inventori transaksi

            refreshLaporan(); // kalau admin buka laporan, data sudah benar
        });
    ...
    private void isiInventoriKeModel(DefaultTableModel modelInventori) {
        for (Buku b : tokoBuku.getInventoriBuku()) {
            modelInventori.addRow(new Object[]{
                    b.getIdBuku(),
                    b.getJudul(),
                    b.getHarga(),
                    b.getStok(),
